...
class CharacterAttributeIntelligence(models.Model):
    ...
    def _check_value_range(self):
        for record in self:
            if (
                record.intelligence_value < record.intelligence_min_value
                or record.intelligence_value > record.intelligence_max_value
            ):
                raise ValidationError(
                    "O valor do atributo Inteligência deve estar entre %s e %s."
                    % (record.intelligence_min_value, record.intelligence_max_value)
                )
