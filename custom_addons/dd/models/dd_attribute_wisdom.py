...
class CharacterAttributeWisdom(models.Model):
    ...
    wisdom_name = fields.Char(
        string="Attribute Name", default="Sabedoria", readonly=True
    )
    ...
    def _check_value_range(self):
        for record in self:
            if (
                record.wisdom_value < record.wisdom_min_value
                or record.wisdom_value > record.wisdom_max_value
            ):
                raise ValidationError(
                    "O valor do atributo Sabedoria deve estar entre %s e %s."
                    % (record.wisdom_min_value, record.wisdom_max_value)
                )
