...
class CSMatch(models.Model):
    ...
    def create_from_json(self, match_json):
        ...
        existing_match = self.env["csg.match"].search(
            [("match_id", "=", match_id)], limit=1
        )
        ...
        if existing_match:
            vals_to_update = {
                ...
                "serie_begin_at": self.convert_datetime(
                    match_json["serie"]["begin_at"]
                ),
                ...
                "opponents": [
                    (
                        ...
                        0,
                        [
                            self.env["csg.opponent"].create_from_json(opponent_json).id
            ...
            vals_to_update = {
                key: val for key, val in vals_to_update.items() if val is not None
            }
